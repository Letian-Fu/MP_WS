...
void ArmModel::sphereCenters(
    ...
    if (J_point_conf)
        fk_model_.forwardKinematics(jp, boost::none, link_poses, boost::none, J_pose_jp, boost::none, boost::none);
    else
        fk_model_.forwardKinematics(jp, boost::none, link_poses, boost::none, boost::none, boost::none, boost::none);
    ...
    for (size_t sph_idx = 0; sph_idx < nr_body_spheres(); sph_idx++) {
        if (J_point_conf) {
            gtsam::Matrix J_point_pose;
            sph_centers[sph_idx] =
                link_poses[body_spheres_[sph_idx].link_id].transformFrom(
                    body_spheres_[sph_idx].center, J_point_pose);
            (*J_point_conf)[sph_idx] =
                J_point_pose * J_pose_jp[body_spheres_[sph_idx].link_id];
        } else {
            sph_centers[sph_idx] =
                link_poses[body_spheres_[sph_idx].link_id].transformFrom(
...
gtsam::Point3 ArmModel::sphereCenter(
    ...
    if (J_point_conf)
        fk_model_.forwardKinematics(jp, boost::none, link_poses, boost::none,
                                    J_pose_jp, boost::none, boost::none);
    else
        fk_model_.forwardKinematics(jp, boost::none, link_poses, boost::none, boost::none, boost::none, boost::none);
...
gtsam::Matrix ArmModel::sphereCentersMat(const Pose& jp) const {
