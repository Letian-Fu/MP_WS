...
{
    Arm::Arm(size_t dof, const Eigen::VectorXd& a, const Eigen::VectorXd& alpha, const Eigen::VectorXd& d,
        const gtsam::Pose3& base_pose, const Eigen::VectorXd& theta_bias):
        a_(a),
        alpha_(alpha),
        d_(d),
    ...
    void Arm::forwardKinematics(
        ...
        boost::optional<std::vector<Eigen::MatrixXd>&> J_jpx_jp,
        boost::optional<std::vector<Eigen::MatrixXd>&> J_jvx_jp,
        boost::optional<std::vector<Eigen::MatrixXd>&> J_jvx_jv) const {
        using namespace std;
        // space for output
        jpx.resize(dof());
        if (jvx) jvx->resize(dof());
        if (J_jpx_jp) J_jpx_jp->assign(dof(), Eigen::MatrixXd::Zero(6, dof()));
        if (J_jvx_jp) J_jvx_jp->assign(dof(), Eigen::MatrixXd::Zero(3, dof()));
        if (J_jvx_jv) J_jvx_jv->assign(dof(), Eigen::MatrixXd::Zero(3, dof()));

        // variables
        vector<Eigen::Matrix4d> H(dof());
        vector<Eigen::Matrix4d> Ho(dof() + 1);  // start from 1
        vector<Eigen::MatrixXd> J;
        if (jv) J.assign(dof(), Eigen::MatrixXd::Zero(3, dof()));
        // vars cached for calculate output Jacobians
        vector<Eigen::Matrix4d> dH(dof());
        vector<Eigen::Matrix4d> Hoinv(dof() + 1);  // start from 1
        ...
        // zeros
        vector<vector<Eigen::Matrix4d>> dHo_dq(dof(), vector<Eigen::Matrix4d>(dof()));
        ...
        for (size_t i = 0; i < dof(); i++) {
            ...
            if (J_jpx_jp) {
                Eigen::MatrixXd& Jp = (*J_jpx_jp)[i];
                // Jp.setZero();         // when j > i all are zeros
                // Jp each col
                const Eigen::Matrix4d inv_jpx_i = jpx[i].inverse().matrix();
                for (size_t j = 0; j <= i; j++) {
                    const Eigen::Matrix4d sym_se3 = inv_jpx_i * dHo_dq[i][j];
                    Jp.col(j) =
                        (Eigen::VectorXd() << Eigen::Vector3d(sym_se3(2, 1), sym_se3(0, 2), sym_se3(1, 0)),
            ...
            // diff Jvj to vJp
            if (J_jvx_jp) {
                Eigen::MatrixXd& Jv = (*J_jvx_jp)[i];
                // Jv.setZero();
                // Jv each col <= j (j <= i)
                // Jv.col(j) = dvxi_dq.col(j) = d_Ji_qj * vi
                for (size_t j = 0; j <= i; j++) {
                    Eigen::MatrixXd d_Ji_qj = Eigen::MatrixXd::Zero(3, dof());

                    // for d_Ji_qj only first i cols have values
                    // d_Ji_qj.col(k) = d(getJvj(i, k-1))_dqj  (k <= i)
                    d_Ji_qj.col(0) =
                        getdJvj(Ho[i + 1], Ho[0], dHo_dq[i][j], Eigen::Matrix4d::Zero());  // k-1 < 0, use zero for dH-1
                    for (size_t k = 1; k <= i; k++) {
                        if (k - 1 >= j)
                            ...
                            d_Ji_qj.col(k) =
                                getdJvj(Ho[i + 1], Ho[k], dHo_dq[i][j], Eigen::Matrix4d::Zero());  // zero dHo_dq when j > k-1
                    }
                    Jv.col(j) = d_Ji_qj * (*jv);
                }
            }
